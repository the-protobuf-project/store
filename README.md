...
| `license_header` | Path to a file whose lines are placed above the banner of every generated file — a copyright and `SPDX-License-Identifier` block, typically. Write the lines **without** comment markers: each target applies its own (`//` for Go, Prisma and TypeScript, `--` for SQL), so one file covers the whole output tree. Unset (the default) stamps nothing, because generated code belongs to whoever ran the generator, not to this plugin. |
